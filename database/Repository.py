...
class UserRepository(BaseRepository):
    ...
    def __verificar_se_produto_listado_existe(self, id_produto_listado: int) -> bool:
        query = "SELECT 1 FROM produtos_listados WHERE id_produto_listado = %s;"
        try:
            cursor = self.database.cursor
    ...
    #Tested
    ## Pre_condição - residencia referente ao metodo tem que existir
    def alterar_adm(self, id_residencia:int, id_novo_adm:int):
        if self.__verificar_se_residencia_existe(id_residencia):
    ...
    def mostrar_residencia(self, id_residencia:int) -> dict:
        if self.__verificar_se_residencia_existe(id_residencia):
            ...
            raise RuntimeError("ERRO: a residencia de ID {id_residencia} não existe")
    ...
    def deletar_produto_residencia(self, id_produto_residencia:int) -> bool:
        if self.__verificar_se_produto_residencia_existe(id_produto_residencia):
            ...
            raise RuntimeError("ERRO: o produto da residencia solicitado não existe")
    ...
    ##
    def adicionar_produto_lista_pessoal(self, id_produto_residencia, id_lista, quantidade):
        query = "INSERT INTO produtos_listados (produto_residencia_id, lista_id, quantidade_listada) VALUES (%s, %s, %s);"
        try:
            self.database.cursor.execute(query, (id_produto_residencia, id_lista, quantidade))
            self.database.connection.commit()
        except mysql.connector.Error as err:
            raise RuntimeError(f"Erro ao registrar produto na lista pessoal de ID {id_lista}: {str(err)}")
    ...
    def deletar_produto_lista_pessoal(self, id_produto_listado):
        if self.verificar_se_produto_listado_existe(id_produto_listado):
            query = "DELETE FROM produtos_listados WHERE id_produto_listado = %s;"
            try:
                cursor = self.database.cursor
                cursor.execute(query, (id_produto_listado,))
                self.database.connection.commit()
            ...
            raise RuntimeError("ERRO: o produto listado de ID {id_produto_listado} não existe")
    
    ## Pre_condição - usuario referente ao metodo tem que existir
    def mostrar_produtos_listados_do_usuario(self, id_usuario: int) -> dict:
        if self.__verificar_se_usuario_existe(id_usuario):
            query = '''
            SELECT id_produto_listado, id_produto_residencia, residencia_id, lista_id, quantidade_listada, nome, quantidade_unid, unidade_de_medida, categoria, preco_medio
            FROM produtos_listados
            INNER JOIN produtos_residencia 
            ON produtos_listados.produto_residencia_id = produtos_residencia.id_produto_residencia
            WHERE lista_id = %s;'''
            try:
                cursor = self.database.cursor
                cursor.execute(query, (id_usuario,))
    ...
    def registrar_usuario_que_consome_um_produto(self, id_produto_residencia:int, id_usuario:int) -> None:
        query = f"INSERT INTO produtos_residencia__usuarios (produto_residencia_id, usuario_id) VALUES ('{id_produto_residencia}', '{id_usuario}');"
    ...
    def deletar_usuario_que_consome_um_produto(self, id_produto_residencia:int, id_usuario:int):
        ...
        if self.__verificar_se_produto_residencia_e_usuario_existe(id_produto_residencia, id_usuario):
            ...
            raise RuntimeError("ERRO: o usuario de ID {id_usuario} não consome o produto de ID {id_produto_residencia}")
        
    ## Pre_condição - produto da residencia referente ao metodo tem que existir
    def mostrar_usuarios_que_consomem_certo_produto(self, id_produto_residencia: int) -> dict:
        if self.__verificar_se_produto_residencia_existe(id_produto_residencia):
    ...
    ##
    def registrar_compra(self, id_residencia: int, id_usuario: int, supermercado: str) -> bool:
        data = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S") # Formato de data compatível com MySQL
        query = "INSERT INTO compras (residencia_id, usuario_id, data_compra, supermercado) VALUES (%s, %s, %s, %s)"
        try:
            self.database.cursor.execute(query, (id_residencia, id_usuario, data, supermercado))
            self.database.connection.commit()
        except mysql.connector.Error as err:
    ...
    def deletar_produto_da_dispensa(self, id_produto_dispensa:int) ->bool:
        if self.__verificar_se_produto_dispensa_existe(id_produto_dispensa):
            ...
            raise RuntimeError("ERRO: o produto da dispensa de ID {id_produto_dispensa} não existe")
    ...
    def mostrar_produtos_dispensa(self, id_residencia: int) -> dict:
        if self.__verificar_se_residencia_existe(id_residencia):
            ...
            INNER JOIN produtos_residencia ON produtos_comprados.produto_residencia_id = produtos_residencia.id_produto_residencia
            WHERE produtos_residencia.id_produto_residencia = %s;'''
            try:
                self.database.cursor.execute(query, (id_residencia,))
                result = self.database.cursor.fetchall()
                if result:
    ...
    def registrar_dividas(self, id_compra:int, valor:float, id_usuario_beneficiario:int, id_usuario_pagador:int):
       ...
       query = "INSERT INTO dividas (compra_id,valor, beneficiario, pagador, data_divida) VALUES (%s, %s, %s, %s, %s);"
       try:
...
class RepositorioResidencia(BaseRepository):
    def __init__(self, database):
        super().__init__(database)
    ...
    def __verificar_se_usuario_existe(self, id_usuario: int) -> bool:
        ...
        try:
            ...
            raise RuntimeError(f"Erro ao verificar se o usuário de ID {id_usuario} existe no banco: {str(err)}")
    
    
'''

Falta:
    testar codigo novamente, com excessoes
    revisar codigo
    compartilhamento
    integração
